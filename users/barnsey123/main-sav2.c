...
char canbetakenweight=6;	// weight to be applied to escape position if can be taken
...
ClearTargetAndComputer();	// clear target and computer arrays
ClearEnemy();				// enemy is seperate as used elsewhere
...
for (ctns=0;ctns<11;ctns++)
	...
	for (ctew=0;ctew<11;ctew++)
		...
		if (fb>5)			// fb=6 (+COMPUTER array):fb=7 (can I be taken) 
			{
...
//	{
	while ((counter>-1)&&(counter<11))
		...
		if (computer[x][y])	// if accessible by attacker	
			...
			if (z==0) {target[x][y]+=points;}else{target[x][y]=points;}
			}
		else 
			{	
			if (players[x][y]==0)	// if blank)
				{
...
pacpointsy=checkroute(kingns,kingew,0,10);
subpacmanx();
//SOUTH
orientation=1;
paccount1=kingsouth;
...
pacpointsy=checkroute(kingns,kingew,10,10);
subpacmanx();
// EAST
orientation=2;
paclevel1=kingns;
...
if ( xplayers == 0 )	// if adjacent square is OK
	...
	while ( arrow == 1 ) // keep checking until cannot move
		...
		if ( arrow == 1 )						// if MODE is "draw an arrow" (aka: I can move here)
			...
			if (fb==5) {subarrows2();} 		// enemy can get here, update enemy array (direction specific)
			if (fb==6) {computer[xns][xew]++;} // computer can get here, increment computer array 
	...
	if ((fb==7)&&(xplayers>1))	// check to see if an attacker can be caught if he stays where he is
		...
		if ((players[takerow][takecol]==0)&&(enemy[takerow][takecol])&&(enemy[takerow][takecol]<enemyweight)) 
			{
			target[takerow][takecol]+=canbetakenweight;  // update adjacent target to provide escape route or place for someone else to occupy
			}
		}
}
/************************************************/
...
if (surrounded==3)	// unset any "enemy values" 
	{
	ezew1=kingew;
	if ( kingns > 0 )
		{
...
if (( players[ezns1][ezew1] == 0 )&&(target[ezns1][ezew1]))	// if adjacent square n/s/e/w is blank and accessible
		{
		target[ezns1][ezew1]=100;	// set big target value
		ClearEnemy();		// set entire enemy array to zero
		}
}
/******************************/
...
if (target[ctns][ctew])	// only if target is valid (i.e. not a king square)
	{
	/*
	orientA=kingnorth+defnorth;
	orientB=kingsouth+defsouth;
	if (orientA>orientB) { orient=1;orientA=orientB;}
	orientB=kingeast+defeast;
	if (orientA>orientB) { orient=2;orientA=orientB;}
	orientB=kingwest+defwest;
	if (orientA>orientB) { orient=3;orientA=orientB;}
	if ( orientA < 2 )
		{
		if (ctns==kingns) // increase target if on same level as king	
			{
			if (ctew>kingew) {x=ctew-kingew;}else{x=kingew-ctew;}	// find distance from king
			z=11-x;
			if ( orient==0 )
				{
				target[ctns][ctew]+=z;
				if ((kingwest==0)&&(ctew<kingew)) 	{target[ctns][ctew]+=10;}
				if ((defwest==0)&&(ctew<kingew))	{target[ctns][ctew]+=10;}
				}
			if ( orient==1 )
				{
				target[ctns][ctew]+=z;
				if ((kingeast==0)&&(ctew>kingew))	{target[ctns][ctew]+=10;}
				if ((defeast==0)&&(ctew>kingew))	{target[ctns][ctew]+=10;}
				}
			}	
		if (ctew==kingew) // increase target if on same level as king	
			{
			if (ctns>kingns) {x=ctns-kingns;}else{x=kingns-ctns;}	// find distance from king
			z=11-x;
			if ( orient==2)
				{
				target[ctns][ctew]+=z;
				if ((kingnorth==0)&&(ctns<kingns)) 	{target[ctns][ctew]+=10;}
				if ((defnorth==0)&&(ctns<kingns))	{target[ctns][ctew]+=10;}
				}
			if ( orient==3 )
				{
				target[ctns][ctew]+=z;
				if ((kingsouth==0)&&(ctns>kingns))	{target[ctns][ctew]+=10;}
				if ((defsouth==0)&&(ctns>kingns))	{target[ctns][ctew]+=10;}
				}
			}
		}
	*/	
