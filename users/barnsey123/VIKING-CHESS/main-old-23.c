...
counter=kingns-1;		// check north from King(default to kings ns position -1)
...
if ( xplayers == 0 )	// if adjacent square is OK
	...
	if ((fb==7)&&(xplayers!=1))	// check to see if an attacker can be caught if he stays where he is
		{
		if ((players[takerow][takecol]==0)&&(enemy[takerow][takecol])&&(target[takerow][takecol]>1)) {target[takerow][takecol]+=5;}
		}
	}
}
/************************************************/
...
if ((ns>0)&&(ns<10))
	...
	if ((players[ns-1][ew]==2)||(players[ns-1][ew]==3))
		...
		if ( origorient == 0 ) // if heading NORTH, check SOUTH for DEFENDER
			{
			LookBackInAnger();
			}
		}
	if ((players[ns+1][ew]==2)||(players[ns+1][ew]==3))
		...
		if ( origorient == 1 ) // if heading SOUTH, check NORTH for DEFENDER
			{
			LookBackInAnger();
			}
		}
...
if ((ew>0)&&(ew<10))
	...
	if ((players[ns][ew+1]==2)||(players[ns][ew+1]==3))
		...
		if ( origorient == 2 ) // if heading EAST, check WEST for DEFENDER
			{
			LookBackInAnger();
			}
		}
	if ((players[ns][ew-1]==2)||(players[ns][ew-1]==3))
		...
		if ( origorient == 3 ) // if heading WEST, check EAST for DEFENDER
			{
			LookBackInAnger();
			}
		}			
