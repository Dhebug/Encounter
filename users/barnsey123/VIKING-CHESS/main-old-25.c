...
void subpacmanx(char,char, char, char, char);			// grand sub of pacman
...
void surroundcheck(char, char);		// inc surrounded under various conditions
//void PrintChr();
...
/************************************************/
void subpacmanx(char xorient, char xlevel, char level, char xking, char xdef)
{
char x;
char y;
setpoints();
if ((xlevel<3)||(xlevel>7)) { incpoints(); }
if ((xorient == 0) || (xorient == 3))
	{
	counter=level-1;
	if ( level<5 ) { incpoints();} // add weight to north or west if king in north or west side of board	
	}
if ( (xorient == 1 )||(xorient == 2))
	{ 
	counter=level+1;
	if ( level>5 ) { incpoints();} // add weight to south or east if king in south or east side of board
	}
if ( xking==0 ) { incpoints();}
if ( (xking+xdef)==0 ) { doublepoints(); }
surroundpoints();
if (xorient<2)
	{
	x=counter;
...
if ( (xorient==0) || (xorient==3) )
	...
	while ((players[x][y] != 1 )&&(counter>-1))
		...
		if ( target[x][y]==0) 
			{
			if ( xorient==0) { subpacman();}else{subpacman2();}
			}
		decpoints();
		deccounter();
		if ( xorient == 0 ) {x=counter;}else{y=counter;}
		}
	}
if ( (xorient==1) || (xorient==2) )
	{
	while ((players[x][y] != 1 )&&(counter<11))
		...
		if ( target[x][y]==0) 
			{
			if (xorient==1) {subpacman();}else{subpacman2();}
			}
		decpoints();
		inccounter();
		if ( xorient == 1 ) {x=counter;}else{y=counter;}
		}
	}
}
/************************************************/
...
// NORTH
subpacmanx(0,kingew, kingns, kingnorth, defnorth);
/*
...
*/
//SOUTH
subpacmanx(1,kingew, kingns, kingsouth, defsouth);
/*
...
*/
// EAST
subpacmanx(2,kingns, kingew, kingeast, defeast);
/*
...
*/
// WEST
subpacmanx(3,kingns, kingew, kingwest, defwest);
/*
...
if ( kingns > 0 )	{ surroundcheck(kingns-1,kingew); }
if ( kingns < 10 )	{ surroundcheck(kingns+1,kingew); }
if ( kingew > 0 )	{ surroundcheck(kingns,kingew-1); }
if ( kingew < 10 )	{ surroundcheck(kingns,kingew+1); }
if (surrounded==3)
	{
	if ( kingns > 0 )
		{
...
{
if (players[ns1][ew1]>1)
		{
		//if (( enemy[ns2][ew2]-xvalue > 0 )||(enemy[ns2][ew2]-50 > 50)) // 14-11-2011 
		if (( (enemy[ns2][ew2]-xvalue > 0 )&&(enemy[ns2][ew2]<enemyweight))||(enemy[ns2][ew2]-enemyweight > 0)) // 06-12-2011 
			{
			subcanbetaken();
			}
...
/****************************/
void surroundcheck(char surns, char surew)
{
if (players[surns][surew]==1)		{incsurround();} 	// is attacher n,s,e,w
