...
/* TODO LIST
checkroute function (returns number of pieces on a given route), start/end pos
...
char x;
char y;
char z=paccount1+paccount2;	// count of pieces on route to edge
setpoints();
if ( z==0 )
	{
...
if (surrounded==3)	// unset any "enemy values" 
	{
	ezew1=kingew;ezew2=kingew;
	if ( kingns > 0 )
		{
		ezns1=kingns-1;ezns2=kingns-2;
		enemyzero();
		}
	if ( kingns < 10 )
		{
	...
	ezns1=kingns;ezns2=kingns;
	if ( kingew > 0 )
		{
		ezew1=kingew-1;ezew2=kingew-2;
		enemyzero();
		}
	if ( kingew < 10 )
		{
...
if ( crstartrow==crdestrow )	// if on same row (then check columns)
	...
	for (y=crstartcol;y<=crdestcol; y++)
		{
		if ((players[x][y]) && (players[x][y]<4)) { inccounter();}
		}
	}
