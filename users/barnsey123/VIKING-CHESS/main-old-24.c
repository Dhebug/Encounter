...
void subcanbetaken2(char, char, char, char, char, char); // attempt to reduce memory footprint
//void PrintChr();
...
if ( xplayers == 0 )	// if adjacent square is OK
	...
	if ((fb==7)&&(xplayers!=1))	// check to see if an attacker can be caught if he stays where he is
		{
		if ((players[takerow][takecol]==0)&&(enemy[takerow][takecol])&&(target[takerow][takecol]>0)) 
			{
			target[takerow][takecol]+=5;
...
if ((ns>0)&&(ns<10))
	{
	subcanbetaken2(ns-1,ew,ns+1,ew,1,0);
	/*
	...
	*/
	subcanbetaken2(ns+1,ew,ns-1,ew,5,1);
	/*
	if ((players[ns+1][ew]==2)||(players[ns+1][ew]==3))
		{
...
if ((ew>0)&&(ew<10))
	{	
	subcanbetaken2(ns,ew+1,ns,ew-1,10,2);
	/*	
	...
	*/
	subcanbetaken2(ns,ew-1,ns,ew+1,20,3);
	/*
	if ((players[ns][ew-1]==2)||(players[ns][ew-1]==3))
		{
...
if ( fb==6 )
	...
	if ( enemy[xns][xew] ) { inctarget();};	// ADD WEIGHT TO FRIENDLY SQUARE IF IT CAN BLOCK AN ENEMY)
	/*
...
/****************************/
void subcanbetaken2(char ns1, char ew1, char ns2, char ew2, char xvalue, char xorigorient)
{
if ((players[ns1][ew1]==2)||(players[ns1][ew1]==3))
		{
		if (( enemy[ns2][ew2]-xvalue )||(enemy[ns2][ew2]-50 > 50)) // 14-11-2011 
			{
			subcanbetaken();
			}
		if ( origorient == xorigorient ) // check opposite to direction of travel for defenders
			{
			LookBackInAnger();
			}
		}
