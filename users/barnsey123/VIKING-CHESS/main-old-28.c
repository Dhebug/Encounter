...
if ( paccount1<2)	// if 1 or none attackers  in direction
	{
	while ((players[x][y] != 1 )&&((counter>-1)&&(counter<11)))
		{
		if ((target[x][y]>0)&&(( target[x][y] > 1 )||(z==0)))	
			{ 
			target[x][y]+=points;// inc target only if accessible by attacker
			}
		if (( target[x][y]==0)&&(players[x][y]==0))	// if not accessible by attacker (and blank)
			{
...
if (target[ctns][ctew])	// only if target is valid (i.e. not a king square)
	{
	//computer[xns][xew]++;
	if (enemy[ctns][ctew]){inctarget();}	// increase target if blocking an attacker		
	//ns=ctns;
	//ew=ctew;
	cantake=0;	
	inccantake();
