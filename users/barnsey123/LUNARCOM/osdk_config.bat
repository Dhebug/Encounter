@ECHO OFF

::
:: Set the build paremeters
::
SET OSDKADDR=$500
SET OSDKNAME=CHAPTER1-VIDEOCALL
SET OSDKTAPNAME=VIDEOCALL
SET OSDKBIN=%OSDK%\bin
::SET OSDKFILE=MAIN draw data pictiles explode
::SET OSDKFILE=MAIN draw data pictiles explode runes text font_6x8_runic1_full
::SET OSDKFILE=MAIN draw data pictiles explode border runes
::SET OSDKFILE=MAIN draw data pictiles explode border defence runes font_6x8_runic1_full
SET OSDKFILE=MAIN draw LookRight Tears YouFuck Whoops NodYes LeanForward Yabber Disdain CheckWatch

