...
/****************************************/
/*
14-06-2013 - priority calculations incomplete (use rowcountatt/colcountatt)
...
// 12-06-2013 NB v0.055 implement new highpriority move routine...
#include <lib.h>
#define NORTH 0
#define SOUTH 1
#define EAST 2
...
void prioritynuke();		// nukes priority arrays		
...
unsigned char RowCountAtt[11];	// count of attackers on a Row
unsigned char ColCountAtt[11];	// count of attackers on a Column
...
unsigned char priority,highpriority; // to determine best piece to move
unsigned char highons, highoew;	// the location of the best piece to move
/****************** MAIN PROGRAM ***********************************/
main(){
...
void computerturn(){
  ...
  prioritynuke();	// clears priority arrays
  ...
  for (fb=3;fb<9;fb++){
      ...
      for (ctns=0;ctns<11;ctns++){
        for (ctew=0;ctew<11;ctew++){
          ns=ctns;ew=ctew;
          if (( fb == 3 )&&( players[ctns][ctew] == ATTACKER)){
			RowCountAtt[ctns]++;
			ColCountAtt[ctew]++;
	      }
...
//unsigned char test;
  if ( foundpiece == 0 ){		
	if (players[a][b] == ATTACKER){  // a=row, b=column (ns,ew = target location)
		...
		/*if (foundpiece == 1){ // if after checking it's still a valid candidate...	
			if (origorient < EAST){
				priority=RowCountAtt[a];
			}else{
				priority=ColCountAtt[b];
			}
  ...
  for (ctns=0;ctns<11;ctns++){	// find the highest value for target
    for (ctew=0;ctew<11;ctew++){
      if ( target[ctns][ctew] > hightarget ){
        ...
        targetns=ctns;
        targetew=ctew;		
        //highons=ctns;		// target is accessible so make highons/highoew the default piece position to move
        //highoew=ctew;		// the ACTUAL piece to move determined below (one of ons or oew will remain same)
        //ons=ctns;			// ons/oew also required to be set (various reasons)
        //oew=ctew;
        //ns=ctns;
        //ew=ctew;
      }
...
NEWTARGET:
  ...
  priority=0;highpriority=0;	// initialize prioritys
  //highons=0; highoew=0;
  fb=9;
  // NORTH + SOUTH
  b=targetew;
  ...
  zerofoundpiece();	
  origorient=NORTH;
  for (mkey=targetns-1; mkey>-1; mkey--) {a=mkey;findpiece();}
  // SOUTH
  //zerofoundpiece(); 
  origorient=SOUTH;	
  for (mkey=targetns+1; mkey<11; mkey++)	{a=mkey;findpiece();}
  // EAST + WEST	
  // EAST
  a=targetns;  
  //zerofoundpiece(); 
  origorient=EAST;
  for (mkey=targetew+1; mkey<11; mkey++)	{b=mkey;findpiece();}
  // WEST
  //zerofoundpiece(); 
  origorient=WEST;
  for (mkey=targetew-1; mkey>-1; mkey--)	{b=mkey;findpiece();}	
  if ( highpriority == 0 ) {target[targetns][targetew]=1;goto NEWTARGET;}	// if can still be taken select new target
  //oew=highoew;
  //ons=highons;
  ons=a;
  oew=b;
  ...
  blinkcursor();		// blink cursor at piece position cx,cy
  fb=0;
  //drawcursor();	// blank cursor
  cx=targetew;			// target x screen position
  cy=targetns;			// target y screen position
  blinkcursor();		// blink cursor at target position cx,cy
...
void prioritycalc(){ // calculates the priorities of moving a piece
	for(a=0;a<11;a++){
		for (b=0;b<11;b++){
			if (players[a][b]==ATTACKER){
				// count attackers on row
				for (c=0;c<11;c++){
					if (players[c][b]==ATTACKER) priority[a][b]++;
				}
				// count attackers on a col
				for (d=0;d<11;d++){
					if (players[a][d]==ATTACKER) priority[a][b]++;	
				}
			}
		}
	}
