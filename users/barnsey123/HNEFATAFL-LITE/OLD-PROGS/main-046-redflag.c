...
// 13-05-2013 NB v0.045 Introduction of RED FLAG (vinegar strokes issue 1 resolved)
// 13-05-2013 NB v0.046 Smaller REDFLAG routine.Minor font changes.
/* TO DO:
Vinegar Strokes ISSUE 2: Don't run broken arrow in red flag situation
*/
#include <lib.h>
#define NORTH 0
#define SOUTH 1
...
void setredflag();			// set REDFLAG to YES if all OK
/****************** GLOBAL VARIABLES *******************************/
...
unsigned char redflagtest;		// test used in REDFLAG
/****************** MAIN PROGRAM ***********************************/
main(){
...
void findpiece(){	// find a piece capable of moving to selected target
  if ( foundpiece == 0 ){		
	if ((players[a][b]==ATTACKER)&&(redflag==NO)){  // a=row, b=column
		calccantake();
...
void subpacmanx(){
  ...
  */
  ...
  // Next, determine RED FLAG status (if king has clear straight line path to winning position - or adjacent)
  // Can only set RED FLAG if there are actual targets en-route (kingtoedge)
  if (( kingpieces[orientation] == 0 )&&(kingtoedge[orientation])){
	  //calchightarget();
	  redflagtest=kingew;			// if NORTH/SOUTH
	  if (orientation > SOUTH) redflagtest=kingns; //if EAST/WEST
	  setredflag();
  }
	  /*
...
void setredflag(){
	if (( redflagtest < 2 ) || ( redflagtest > 8 )){
		redflag=YES;
		points=200;
	}
}
...
// improved version of pacman
	...
	for (orientation = 0; orientation < 4; incorientation()){
		...
		if ( orientation ){
			a=10;startrow=10;destrow=10; // SOUTH
			if ( orientation == EAST ){
...
/*
void pacman6(){	// "vinegar strokes" OVERRIDES ALL other points if direct route to corner
	unsigned char test;
	skipcheck=0;
	for (orientation=0; orientation < 4; incorientation() ){
		test=0;
		//tzonemode=VINEGAR;
		if ( kingpieces[orientation] == 0 ){
			if (( orientation == NORTH ) && (kingns > 1) && ((kingew == 0)||(kingew == 10))) test++;
			if (( orientation == SOUTH ) && (kingns < 9) && ((kingew == 0)||(kingew == 10))) test++;
			if (( orientation == EAST ) && (kingew < 9) && ((kingns == 0)||(kingns == 10))) test++;
			if (( orientation == WEST ) && (kingew > 1) && ((kingns == 0)||(kingns == 10))) test++;
			if ( test ){
				skipcheck=1;
				enemy[0][0]=0;
				enemy[0][10]=0;
...
void cantakeadjust(){							
  ...
  if ((playertype == 1)&&(gamestyle == 1)){	// if computer playing as attacker and his turn
    ...
    if (flag )		{
      //if ((kingattacker[orientation]+kingdefender[orientation])<4){cantake--;}
      if (kingpieces[orientation]==1) cantake--; //10-05-2013 if no other pieces on plane
    }
...
void calccantake() {
  ...
  for (x=0;x<4;x++) {
    if ( x<2 ){ // heading north/south
      ...
      incorientation(); inccantake();	// WEST
    }else{ // heading east/west
      orientation=NORTH; inccantake();
      incorientation(); inccantake(); // SOUTH
    }
  }
}
