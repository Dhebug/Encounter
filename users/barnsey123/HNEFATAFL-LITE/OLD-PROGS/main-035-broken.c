...
// 05-05-2013 NB v0.034 Fixed bug in user interface
// 06-05-2013 NB v0.035 brokenarrow twiddles
/* TO DO:
...
void brokenarrownuke();		// kill all targets (promote borkenarrow targets)
/****************** GLOBAL VARIABLES *******************************/
...
void computerturn(){
      ...
      for (ctns=0;ctns<11;ctns++){
        for (ctew=0;ctew<11;ctew++){
          ...
          if ( fb == 4 ){	// fb=4 means: don't print destinations, just update ENEMY	
            if (( players[ctns][ctew] == DEFENDER  )||(players[ctns][ctew] == KING)){ // if enemy found         
               printdestinations();	// update enemy array of possible locations
            }
          }
	      if ((fb == 5)||(fb == 7)){		// fb=5 (+COMPUTER array):fb=7 (can I be taken) 
	          if ( players[ctns][ctew] == ATTACKER){ // if attacker found
	            printdestinations();		
	          }
	      }
	      if ( fb == 6)	{
	      ...
	      if ( fb == 8) {
		      if ( players[ctns][ctew] == KING ) { // if king found
		        printdestinations(); // update kingtracker
		      }
	      }
      }
    }
  ...
  pacman4(); // check for broken arrow AFTER all other points have been calculated
...
void subpacmanx(){
  ...
  */
  ...
  if ((points+(brokenarrow[orientation]*10)) > 250 ){
	  points=250;
	  test2++;
  }else{
  ...
  if ((kingpieces[orientation] == 0)&&((test<2)||(test>8))){	// if no pieces in given orientation and king in the danger zone
	points=150;
	test2++;		
  }
...
void nukebrokenarrow(){ // nukes all other targets except for brokenarrow T-zone
	for (ctns=0;ctns<11;ctns++){
		for(ctew=0;ctew<11;ctew++){
			if (target[ctns][ctew] > 0 ) target[ctns][ctew] = 1;
		}
	}
}
void checkbrokenarrow(){
	...
	if (( c == 0 ) && ( d == 1 )) {
		f=players[a][b];		// check for piece at edge of board
	  	if ( target[a][b] > 0 ) kingtoedge[orientation]--; // decrement kingtoedge[](so maybe kingtoedge[]=zero) - to trigger brokenarrow
		if (( kingpieces[orientation] == 1) && (f > 0) && (f != ATTACKER)) test=1;
		if ((kingpieces[orientation] == 0 )||( test == 1 )){
			if ( kingtoedge[orientation] == 0){	// no targets on route to edge
				setcheckmode3();test2=checkroute();
				if (test2 > 0 ){	// if targets on broken arrow route
...
// improved version of pacman
	...
	for (orientation = 0; orientation < 4; orientation++){
		...
		if ( orientation == SOUTH ){
			a=10;b=kingew;startrow=10;destrow=10;startcol=0;destcol=kingew;
		}
		if ( orientation == EAST ){
...
void pacman4(){
	...
	a=0;b=kingew;startrow=1;destrow=1;startcol=0;destcol=10;
	pacman3();
	if ( brokenarrow[orientation] == 5 ) return;
	orientation=SOUTH;
	a=10;startrow=9;destrow=9;
	pacman3();
	if ( brokenarrow[orientation] == 5 ) return;
	orientation=EAST;
	pacman5(); // set paclevels
  ...
  */
  ...
  while (turn){			// repeat until move is made
    ...
    if (( xkey == 88) || ( xkey == 80)){	// if 'X' or 'P' is selected (88=X, 80=P)
      ...
      if (( playertype == ATTACKER )&&(players[ns][ew] == ATTACKER ))	canselect=1; // piece is selectable
      if (( playertype == DEFENDER )&&((players[ns][ew] == DEFENDER )||(players[ns][ew] == KING))) canselect=1;// piece is selectable
      if ( canselect > 0  ){
        canpiecemove();
        if (route > 0) { 
      ...
      if (( mkey == 88 )&&( canselect > 0 )){	// if piece is SELECTED and CAN move
  ...
  for (x=checkroutestart;x<=checkroutedest;x++){
		...
		switch(checkroutemode){
    		case 1:	if ((players[checkrouterow][checkroutecol] == ATTACKER )||(players[checkrouterow][checkroutecol] == DEFENDER )) z++;break;
      		...
      		case 7: computer[checkrouterow][checkroutecol]=target[checkrouterow][checkroutecol];break;
      		case 8: target[checkrouterow][checkroutecol]=computer[checkrouterow][checkroutecol];break;
  		}
  		if ( startrow == destrow ) {checkroutecol++;}else{checkrouterow++;}
...
void subzoneupdate(){	// (updates border targets)
	...
	checkroutemode=7;checkroute();	// set COMPUTER VALUE = TARGET VALUE prior to nuking target
	nukebrokenarrow(); // nuke all other targets
	checkroutemode=8;checkroute();	// set computer value back to target value
}
