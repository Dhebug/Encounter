...
// 19-12-2013 NB v0.103 restored from v092 added bits from intervening versions
/****************************************/
// TODO:
...
void Xenon2();				// subroutine of pacman4
/****************** GLOBAL VARIABLES *******************************/
...
void checkbrokenarrow(){
	...
	if (( kingpieces[orientation] == 1) && (f)) {kingpieces[orientation]=0;c--;}
	if (( c == 0 ) && ( d )) {
		// decrement kingtargets(so maybe kingtargets=zero) - to trigger brokenarrow
	  	if (( target[a][b] )&&(tzonemode==FULL)) kingtargets[orientation]--; 
...
void pacman2b(){
	...
	// Check the whole OUTER row (forget partial)
	//a=0;b=kingew;
	x=kingns;y=0;startrow=0;destrow=0;startcol=0;destcol=10; // NORTH
	if ( orientation ){
		//a=10;b=kingew;
		x=kingns;y=10;startrow=10;destrow=10;startcol=0;destcol=10; // SOUTH
		if ( orientation == EAST ){	// EAST
			//a=kingns;b=10;
			x=kingew;y=10;startrow=0; destrow=10;startcol=10;destcol=10;
		}
		if ( orientation == WEST ){	// WEST
			//a=kingns;b=0;
			x=kingew;y=0;startrow=0; destrow=10;startcol=0; destcol=0;
		}
	}
	pacman2c();
	// Check the whole INNER row
	x=kingns;y=1;startrow=1;destrow=1;startcol=0;destcol=10; // NORTH
	if ( orientation ){
		x=kingns;y=9;startrow=9;destrow=9;startcol=0;destcol=10; // SOUTH
		if ( orientation == EAST ){
			x=kingew;y=9;startrow=0; destrow=10;startcol=9;destcol=9;
		}
		if ( orientation == WEST ){
			x=kingew;y=1;startrow=0; destrow=10;startcol=1; destcol=1;
		}
	}
	pacman2c();	
...
void pacman2c(){
	if ( (((orientation==NORTH)||(orientation==WEST))&&(x>y)) ||(((orientation==SOUTH)||(orientation==EAST))&&(x<y)) )  checkbrokenarrow();
}
/*
void pacman3(){
	//unsigned char test=0;
