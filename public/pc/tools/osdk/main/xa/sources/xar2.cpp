...
int rd_set(int pc, int afl, int l, int lab) 
{
	...
	{
	  if((afl & A_FMASK) != (eSEGMENT_ZERO<<8)) errout(W_ADRRELOC);
	...
	{
	  afile->rd.mlist+=500;
	  afile->rd.rlist=(relocateInfo*)realloc(afile->rd.rlist, afile->rd.mlist*sizeof(relocateInfo));
	}
	if(!afile->rd.rlist)
	{
...
int rd_write(FILE *fp, int pc) 
{
	...
	/* hack to switch undef and abs flag from internal to file format */
	  ...
	  else 
	  {
	    ...
	    pc=pc2;
	    fputc((afl>>8)&255, fp);
            if((afile->rd.rlist[p].afl&A_FMASK)==(eSEGMENT_UNDEF<<8)) 
			{
