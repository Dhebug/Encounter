...
// MAZE.S
//
extern	unsigned char	GetType(int x,int y);
...
unsigned char	*LevelOffset=&Map[0][0][0];
...
void main()
{
    ...
    Load("BUMPER.HIR");     // Logo NeXT
    LoadAdr("ANCIENT.FNT","#9800");

    Wait(50);
    Load("TITLE.HIR");      // Titre du jeu
    Wait(50);
	...
	if (car)
	{
		...
		AddMessage("",BLACK);
		AddMessage("",BLACK);
		AddMessage("Press any key to continue",YELLOW);
		get();
		hires();
		display_menus();
		break;
	...
	Map[0][3][3]=2-Map[0][3][3];
	Map[0][2][3]=2-Map[0][2][3];
	//xxxxxxxxxxxxxxxxxxxxxxxxxxxx
	display_map();
	display_3d();
	FlagMove=FALSE;
