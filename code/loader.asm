...
_LoaderAddress=*+1
_LoaderAddressLow=*+1
_LoaderAddressHigh=*+2
    jsr LOADER_KERNEL_PROGRAM_ADDRESS  ; Give control to the application and hope it knows what to do
    jmp _LoaderResidentStart
...
auto_fdc_flags_1
    ...
    ; Starting track	
    ldy #%00000000 			; Side 0
_LoaderFileStartTrack=*+1
    lda #LOADER_KERNEL_PROGRAM_TRACK  ; If the track id is larger than 128, it means it is on the other side of the floppy
    bpl first_side
...
_LoadData
    ...
    clc
    lda _LoaderAddressLow
    sta ptr_destination+0
_LoaderFileSizeLow=*+1
    adc #<LOADER_KERNEL_PROGRAM_SIZE
    sta ptr_destination_end+0
    
    lda _LoaderAddressHigh
    sta ptr_destination+1
_LoaderFileSizeHigh=*+1
    adc #>LOADER_KERNEL_PROGRAM_SIZE
...
; As well as _LoaderAddress
; It fills the rest
_InitializeFileFromDirectory
_LoaderEntryIndex=*+1
    ldx #0                      ; operand self-modified by the kernel = file directory index

	lda FileStartSector,x
	sta _LoaderFileStartSector

	lda FileStartTrack,x
	sta _LoaderFileStartTrack

	lda FileSizeLow,x
	sta _LoaderFileSizeLow

	lda FileSizeHigh,x
	sta _LoaderFileSizeHigh
    rts
...
; Assumes that _LoaderEntryIndex contains a valid value (temp)
; As well as _LoaderAddress
; It fills the rest
...
_LoaderFileStartSector   .byt LOADER_KERNEL_PROGRAM_SECTOR       ; $FFF3
...
;
    ...
    .dsb $FFFA - _EndLoaderCode

;
; These three HAVE to be at these precise addresses, they map to hardware registers
;
...
#print (_VectorNMI - _EndLoaderCode)
#if ( (_VectorNMI - _EndLoaderCode) < 0 )
